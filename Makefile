...
CC = gcc
CXX = g++
CFLAGS = -I$(BIN_DIR) -L$(BIN_DIR) -ltulip
CXXFLAGS = -I$(BIN_DIR) -L$(BIN_DIR) -ltulip
...
# Build the shared library (libtulip.so)
.PHONY: lib
lib:
	@mkdir -p $(BIN_DIR)
...
build-benchmark:
	@mkdir -p $(BIN_DIR)
	@go build -o $(BIN_DIR)/benchmark_tulip benchmark/benchmark.go
	@echo "✅ Benchmark build complete! Binary located at $(BIN_DIR)/benchmark_tulip"
...
run-benchmark: build-benchmark
	@echo "🚀 Running benchmark_tulip..."
	@$(BIN_DIR)/benchmark_tulip
	@echo "🏁 Execution finished!"
...
help:
	...
	@echo "  make build-benchmark - Build the benchmark binary into $(BIN_DIR)/benchmark_tulip"
